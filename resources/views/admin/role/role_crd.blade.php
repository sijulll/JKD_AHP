        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Role Settings</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
              ...
              <td>{{$role->deskripsi}}</td>
                {{-- <td>
                    ...
                    <form action="{{ route('role.destroy', $role->id) }}" method="post">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                <a href="{{ route('role.edit',$role->id)}}" class="btn btn-warning show-modal" data-id="{{$role->id}}" data-nama="{{$role->role}}" data-deskripsi="{{$role->deskripsi}}" >
                        Edit
                </a>
                    <button type="submit" data-id="{{$role->id}}" data-nama="{{$role->role}}" data-deskripsi="{{$role->deskripsi}}" class="btn btn-danger" >
                        Delete
                    </button>
                    </form>
                </td> --}}
...
{{-- <div class="modal fade" id="modal-create">
    ...
    <form action="{{route('role.store')}}" method="POST">
        ...
        <div class="form-group">
            <label for="role">Nama Jenis Kegiatan</label>
            <input type="text" class="form-control" id="role" name="role" placeholder="Nama Jenis Kegiatan">
        </div>
        <div class="form-group">
